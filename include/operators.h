...
// Diagnostics
amrex::MultiFab compute_energy(const amrex::MultiFab& B_c, const amrex::MultiFab& E_c);

// Boundary Conditions
void node_period(amrex::MultiFab& mf, const amrex::Periodicity& period);

// Curl operators
std::array<amrex::MultiFab,3> curl_e2f(const std::array<amrex::MultiFab,3>& edge_data, const amrex::GpuArray<amrex::Real,3>& dx);
amrex::MultiFab curl_f2n(const std::array<amrex::MultiFab,3>& face_data, const amrex::GpuArray<amrex::Real,3>& dx);
