...
parser.add_argument("-x", "--xZoom", type = float, default = 1,
                    help = "Zoom in x-axis by given factor towards origin")
parser.add_argument("-y", "--yZoom", type = float, default = 1,
...
def doPlot():
   ...
   dk = 2*math.pi/(dx*nx)
   domega = 2*math.pi/(time_step*ntic)
   ...
   if args.Whistler == 1:
      whistler_p_r = whistler(omega, right = True, hybrid = True, kmax = kmax)
      whistler_p_l = whistler(omega, right = False, hybrid = True, kmax = kmax)
   else:
      whistler_p_r = whistler(omega, right = True, hybrid = False, kmax = kmax)
      whistler_p_l = whistler(omega, right = False, hybrid = False, kmax = kmax)
...
def plotter(data, filename, title, kLims, omegaLims, k, omega, dk, domega, whistlers = None):
   ...
   # ax.axhline(9.21034037198e7, color='g')
...
def getvA(B_dat, ne_dat, t_pt):
   ...
   B_dat2 = B_dat[t_pt,:,0].data**2
   ne_dat = ne_dat[t_pt,:].data
...
def getParam(B_dat, ne_dat, n_dat, T_dat, Ue_dat = None):
   ...
   av_tmp = [x/y for x,y in zip(list(av_tmp.flat), list(ne_dat.data.flat)) if y > 0]
...
xr_data = xr.open_dataset("data.nc", engine = "h5netcdf")

nt = xr_data.timestep[-1].data.item()
dtic = xr_data.timestep.diff("t").mean().data.item()
dt = xr_data.t[-1].data.item()/nt
...
n_list = [i for i in var_name if i.startswith("rho_")]
...
ntic = xr_data.t.shape[0]

tmp1 = xr_data.t.min().item()
tmp2 = xr_data.t.max().item()

tInterval = (tmp2 - tmp1)
time_step = tInterval/(ntic - 1)
...
EM_loc = var_name.index("EM_Energy")

n_dataset = np.zeros(var_data[EM_loc].shape)
v_dataset = np.zeros(var_data[EM_loc].shape)
T_dataset = np.zeros(var_data[EM_loc].shape)

n_dataset = n_dataset[np.newaxis,:,:]
v_dataset = v_dataset[np.newaxis,:,:]
T_dataset = T_dataset[np.newaxis,:,:]
...
if not n_tmp:
   n_tmp.append(np.zeros(var_data[EM_loc].shape))
...
var_name.append("rho_0")
var_data.append(xr.zeros_like(var_data[EM_loc]))
var_real.append(True)

B_loc = var_name.index("B")
ne_loc = var_name.index("rho_0")
Ue_present = False
if "cellUe" in var_name:
   Ue_present = True
   Ue_loc = var_name.index("cellUe")

vA = getvA(var_data[B_loc], var_data[ne_loc], ntic-1)

v_dataset = np.zeros(list(n_dataset.shape) + [3])
T_dataset = np.zeros(n_dataset.shape)

v0 = getv0(n_dataset, v_dataset, ntic-1)
v0 = v0[0]
...
for tmp_name, tmp_data, tmp_real in zip(var_name, var_data, var_real):
   if tmp_data.ndim == 3 and tmp_data.shape[-1] == 3:
...
window = repeat_stack(np.hamming(ntic), nx, 1)
...
if args.Mirror == 0:
   ...
   c2 = ntic
   if args.yZoom>1:
      c2 = c1 + int((ntic+2)/(2*args.yZoom))
   if args.Logarithmic:
      c1 += 1
else:
   if args.yZoom>1:
      c2 = ntic - int(ntic/2 - (ntic/2)/args.yZoom)
   else:
      c2 = ntic
   c1 = ntic - c2
